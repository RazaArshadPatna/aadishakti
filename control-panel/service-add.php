...
if(isset($_GET['id'])){
    ...
    $id=$_GET['id'];
	$edit_id=$_GET['id'];
    $sql="SELECT `id`,`name`, `mrp`, `price`, `details`,`media` FROM `service` where id=?";
	 $s=$conn->prepare($sql);
	 $s->bind_param("s",$id);
	 $s->bind_result($id,$name,$mrp,$price,$details,$image);
	 if($s->execute()){
		$s->fetch();
		$s->close();
}}
      ...
      <div class="container">
        <div class="row g-3 mb-3 align-items-center">
          ...
          <div class="col text-md-end">
            <a class="btn btn-primary" href="service-dis.php"><i class="fa fa-long-arrow-left me-2"></i>View</a>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="row g-3 clearfix row-deck">
          <div class="col-lg-12 col-md-12">
            <div class="card">
			  ...
			  <form id="myForm" role="form" action="action/service.php" method="post" enctype="multipart/form-data">
              <div class="card-body">
                ...
                <div class="col-md-12">
                  ...
                  <?php if($auto_Fill){ ?>
                  <div class="col-md-12">
                      <img class="img-table" src="../upload/service/<?php echo $image; ?>" width="50" height="50" />
				...
				<div class="col-md-12">
                  <label for="exampleInputEmail1">MRP<span class="label label-danger" id="pf_error"></span></label>
                  <input type="text"  required="" name="mrp" value="<?php if(!empty($mrp)){echo $mrp;}?>"  class="form-control" />
            
                </div>
                  <div class="col-md-12">
                  <label for="exampleInputEmail1">Price<span class="label label-danger" id="pf_error"></span></label>
                  <input type="text"  required="" name="price" value="<?php if(!empty($price)){echo $price;}?>"  class="form-control" />
        ...
        canvas.toBlob(function(blob) {
            ...
            reader.onloadend = function() {
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "./action/crop_image_service.php",
