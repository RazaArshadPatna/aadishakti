...
function save($conn,$cat_id,$heading1,$title,$status,$entry_time,$entry_by,$media){
    $sql="INSERT INTO `master_post`(`cat_id`,`heading1`,`title`,`status`,`entry_time`,`entry_by`,`media`) VALUES (?,?,?,?,?,?,?)";
    $s=$conn->prepare($sql);
...
function update($conn,$heading1,$title,$status,$entry_time,$entry_by,$media,$id){ 
    //echo $title;exit;
    $sql="update `master_post` set `heading1`=?,`title`=?,`status`=?,`entry_time`=?,`entry_by`=?,`media`=? where id=?";
    $s=$conn->prepare($sql);
...
if(isset($_POST['head1'])){
                $width=4; $table_name="master_post";$prefix="PS";
