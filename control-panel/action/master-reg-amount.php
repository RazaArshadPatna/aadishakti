...
function save($conn,$cat_id,$heading1,$amount,$status,$entry_time,$entry_by,$media){
    //echo $amount;exit;
    $sql="INSERT INTO `master_reg_amount`(`cat_id`,`heading1`,`amount`,`status`,`entry_time`,`entry_by`,`media`) VALUES (?,?,?,?,?,?,?)";
    $s=$conn->prepare($sql);
...
function update($conn,$heading1,$amount,$status,$entry_time,$entry_by,$media,$id){ 
    //echo $amount;exit;
    $sql="update `master_reg_amount` set `heading1`=?,`amount`=?,`status`=?,`entry_time`=?,`entry_by`=?,`media`=? where id=?";
    $s=$conn->prepare($sql);
...
if(isset($_POST['head1'])){
                $width=4; $table_name="master_reg_amount";$prefix="REG";
