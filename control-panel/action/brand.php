...
function save_brand($conn,$image,$heading1,$heading2,$heading3,$show_button,$url,$status,$entry_by,$entry_time,$city_id){
    $sql="INSERT INTO `brand`(`image`, `heading1`, `heading2`, `heading3`, `show_button`, `url`, `status`, `entry_by`, `entry_time`, `city_id`) VALUES (?,?,?,?,?,?,?,?,?,?)";
    $s=$conn->prepare($sql);
...
function update_brand($conn,$image,$heading1,$heading2,$heading3,$show_button,$url,$status,$entry_by,$entry_time,$city_id,$id){ 
    $sql="update `brand` set `image`=?, `heading1`=?, `heading2`=?, `heading3`=?, `show_button`=?, `url`=?, `status`=?, `entry_by`=?, `entry_time`=?, `city_id`=? where id=?";
    $s=$conn->prepare($sql);
...
if(isset($_POST['head1'])){
        ...
        if($logo_70==""){
            $logo_70=$old_logo_70;
             ...
             $p= update_brand($conn, $logo_70, $heading1, $heading2, $heading3, $show_button, $url, $status, $entry_by, $entry_time, $city_id,$id);
        }else{
            $p= save_brand($conn, $logo_70, $heading1, $heading2, $heading3, $show_button, $url, $status, $entry_by, $entry_time, $city_id);
        }
