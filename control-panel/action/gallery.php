...
function save_static($conn,$cid,$heading,$url,$image,$entry_time,$entry_by,$status){
    $sql="INSERT INTO `manage_gallery`(`cid`,`heading`,`url`,`image`, `cdate`, `cby`, `status`) VALUES (?,?,?,?,?,?,?)";
    $s=$conn->prepare($sql);
...
function update_static($conn, $cid,$heading,$url,$image,$entry_time,$entry_by,$status,$id){ 
   
    $sql="update `manage_gallery` set `cid`=?,`heading`=?,`url`=?,`image`=?,`cdate`=?,`cby`=?,`status`=? where id=?";
    $s=$conn->prepare($sql);
    $s->bind_param("ssssssss",$cid,$heading,$url,$image,$entry_time,$entry_by,$status,$id);
    if($s->execute()){
        return true;
    } else {
        return FALSE;
        ...
        if(isset($_POST['Edit_id'])){
			...
			$p=update_static($conn, $cid,$heading,$url,$image,$entry_time,$entry_by,$status,$id);
        }else{
