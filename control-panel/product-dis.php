...
function get_cat($conn,$cat_id){
  $sql = "SELECT `heading1` FROM `master_category` WHERE `cat_id`='".$cat_id."'";
  $st=$conn->prepare($sql);
  $st->bind_result($name);
  $st->execute();
  $st->fetch();
      ...
      <div class="container">
        <div class="row g-3 mb-3 align-items-center">
          ...
          <div class="col text-md-end">
            <a class="btn btn-primary" href="product-add.php"><i class="fa fa-plus me-2"></i>Add</a>
            
            <button class="btn btn-danger btn-flat" onClick="Trush('product')"><i class="fa fa-trash"></i></button>
          </div>
        </div>
      </div>
    </div>
            ...
            <div class="card">
              ...
              <tbody>
                 <?php 
                    ...
                    $sql="SELECT `id`, `name`, `cat_id`,`media`, `cdate`, `cby` FROM `product`";
                            $s=$conn->prepare($sql);
                            $s->bind_result($id,$name,$cat_id,$img,$cdate,$cby);
                            if($s->execute()){
                              $s->store_result();
                      while($s->fetch()){
