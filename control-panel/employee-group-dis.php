...
function get_count_member($conn,$grp_id){
  $sql = "SELECT `grp_id` FROM `farmer_registration` WHERE `grp_id`='".$grp_id."'";
  $st = $conn->prepare($sql);
  $st->execute();
  $st->store_result();
  $count = $st->num_rows();
      ...
      <div class="container">
        <div class="row g-3 mb-3 align-items-center">
          ...
          <div class="col text-md-end">
            ...
            <button class="btn btn-success btn-flat" onClick="location.href='action/export-group-list.php'">Export</button>
          </div>
        </div>
      </div>
    </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                <tbody id="result">
                 <?php 
                 $i=1;
                 $sql="SELECT `id`,`entry_by`,`grp_id`,`farmer_grp_id`,`grp_name`,`consultant_name`,`consultant_code`,`entry_date`,`status` FROM `farmer_group` WHERE 1";
                 $s=$conn->prepare($sql);
                 $s->bind_result($id,$entry_by,$grp_id,$fgrp_id,$grp_name,$consultant_name,$consultant_code,$entry_time,$status);
                 if($s->execute()){
                     $s->store_result();
                     while($s->fetch()){
                ...
                <tr id="row<?php echo $id; ?>">
                  ...
                  <td class="center"><a href="group-edit.php?id=<?php echo $id; ?>"><button class="btn btn-primary"><i class="fa fa-edit"></i></button></a></td>
                  <td><?php echo $fgrp_id; ?></td>
                  <td><a href="view-members.php?id=<?php echo $grp_id;?>"><?php echo get_count_member($conn,$grp_id);?></a></td>
                  <td><?php echo $grp_name; ?></td>                    
                  <td><?php echo $consultant_name; ?></td>
                  <td><?php echo $consultant_code; ?></td>
