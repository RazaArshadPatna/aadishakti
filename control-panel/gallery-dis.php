...
function category_name($conn,$id){
	$sql="SELECT `name` FROM `manage_category` where id=".$id;
	 $s=$conn->prepare($sql);
	 $s->bind_result($name);
	 if($s->execute()){
		 $s->fetch();
      ...
      <div class="container">
        <div class="row g-3 mb-3 align-items-center">
          ...
          <div class="col text-md-end">
            <a class="btn btn-primary" href="gallery-add.php"><i class="fa fa-plus me-2"></i>Add</a>
            <button class="btn btn-danger btn-flat" onClick="Trush('manage_gallery')"><i class="fa fa-trash"></i></button>
          </div>
        </div>
      </div>
    </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                <tbody id="result">
                 <?php 
                 $i=1;
                  $sql="SELECT `id`, `cid`,`heading`,`url`, `image`, `cdate`, `cby` FROM `manage_gallery`";
                 $s=$conn->prepare($sql);
                 $s->bind_result($id,$cid,$heading,$url,$image,$entry_time,$entry_by);
                 if($s->execute()){
				 	$s->store_result();
                     while($s->fetch()){
                ...
                <tr id="row<?php echo $id; ?>" >
                    ...
                    <!-- <td><?php echo $heading;?></td> -->
					          <td><?php if(!empty($image)){?><img src="../upload/gallery/<?php echo $image; ?>" width="50px" height="50px" ><?php }else {?> VIDEO. URL IS:<?php echo $url;?><?php }?></td>
                    <td><?php echo CusDate($entry_time); ?></td>
                    <td><?php echo $entry_by; ?></td>
                    
					
					<td class="center"><a href="gallery-add.php?id=<?php echo $id; ?>"><button class="btn btn-primary"><i class="fa fa-pencil"></i></button></a></td>
